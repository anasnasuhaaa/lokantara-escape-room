...
export const StageGrid: React.FC<StageGridProps> = ({ currentStage, onStageClick, onReset }) => {
  ...
  return (
    <div className="w-full h-screen flex flex-col items-center justify-center bg-linear-to-br from-black via-red-950 to-black overflow-y-auto py-4 sm:py-6 md:py-8">
      ...
      {/* Stage Grid */}
      <div className="grid grid-cols-1 gap-4 sm:gap-3 md:gap-4 lg:gap-6 px-3 sm:px-4 md:px-6">
        {[1].map((stage) => (
          <button
            key={stage}
            onClick={() => onStageClick(stage)}
            className={`
              relative w-64 h-18 md:h-24 lg:w-100 lg:h-32 rounded-lg font-bold
          ...
          >
            <span className="text-sm sm:text-xl md:text-2xl">
              ...
              {stage == currentStage ? 'Buka Pertanyaan' : 'Berhasil Dilewati'}
            </span>
            <span className="text-xl sm:text-2xl md:text-3xl lg:text-4xl">
              {stage < currentStage ? '◆' : '◇'}
            </span>
            {/* <span className=" text-sm opacity-75">
              {stage == currentStage ? 'Klik untuk membuka' : 'Done mang'}
            </span> */}
          </button>
        ))}
      </div>
